...
function AdminSidebar({ activeTab, setActiveTab, isOpen, setIsOpen, onLogout, userEmail }) {
  const tabs = [
    ...
    { id: "settings", label: "08 SETTINGS", icon: "⚙" },
      { id: "committee_view", label: "09 COMMITTEE VIEW", icon: "📁" },
...
// ----------------------------------------------------
function EventManager({ events, delegates, committees = [], onUpdate, onRefresh }) {
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  >
                    ...
                    {committees.map((c) => (
                      <option key={c.name || c} value={c.name || c} className="bg-[var(--atlas-black)]">{c.name || c}</option>
                    ))}
                  </select>
                </div>
                <Field
...
// ----------------------------------------------------
// Tab Component: CommitteePDFViewer
// ----------------------------------------------------
function CommitteePDFViewer() {
  return (
    <div className="space-y-6 h-full flex flex-col">
      <div className="border-b border-white/5 pb-4 shrink-0">
        <span className="classified-label text-[var(--atlas-cyan)] text-xs block">/ COMMITTEE OVERVIEW</span>
        <h3 className="font-display text-white text-2xl">COMMITTEES & PORTFOLIOS</h3>
        <p className="text-white/40 text-[10px] mt-1 font-mono">
          Viewing the updated committee and portfolio list. Ensure your PDF is placed in the frontend/public/ folder as "committee_list.pdf".
        </p>
      </div>

      <div className="flex-1 glass rounded border border-white/5 p-2 min-h-[600px]">
        <iframe
          src="/committee_list.pdf"
